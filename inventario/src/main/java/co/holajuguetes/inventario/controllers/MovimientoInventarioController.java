...
public class MovimientoInventarioController {
    ...
    public ResponseEntity<MovimientoInventarioResponse> crear(
        ...
        return ResponseEntity.ok(service.crearMovimiento(r));
    }
    ...
    public ResponseEntity<?> buscar(
        ...
        return ResponseEntity.ok(
                PaginaResponse.fromPage(
                        service.buscarMovimientos(r)
                )
        );
    }

    @PostMapping("/siguiente-referencia")
    public ResponseEntity<String> siguiente(
            @RequestBody OperacionRequest r
    ) {
        return ResponseEntity.ok(
                service.avanzarReferencia(r.getTipoMovimiento())
        );
    }

    @PatchMapping
    public ResponseEntity<MovimientoInventarioResponse> editar(
            @RequestBody OperacionRequest r
    ) {
        return ResponseEntity.ok(service.editarMovimiento(r));
    }
